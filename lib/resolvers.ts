...
const resolvers = {
  Query: {
    ...
    restaurants: async () => {
      const [rows] = await db.query<RowDataPacket[]>("SELECT * FROM restaurants");
      return rows.map((row: any) => ({
        id: row.id,
        name: row.name,
    ...
    restaurant: async (_: any, { id }: { id: number }) => {
      const [restaurantRows] = await db.query<RowDataPacket[]>("SELECT * FROM restaurants WHERE id=?", [id]);
  ...
  Mutation: {
    ...
    updateRestaurant: async (_: any, { input }: { input: any }) => {
      await db.query(
        `UPDATE restaurants SET name=?,location=?,description=?,image_url=? WHERE id=?`,
        [input.name, input.location, input.description, input.imageUrl, input.id]
      );

      const [rows] = await db.query<RowDataPacket[]>("SELECT * FROM restaurants WHERE id=?", [input.id]);
      const row = rows[0];
      return { id: row.id, name: row.name, location: row.location, description: row.description, imageUrl: row.image_url };
    },
    ...
    addCategory: async (_: any, { input }: { input: any }) => {
      const [result] = await db.query<ResultSetHeader>("INSERT INTO categories (name) VALUES (?)", [input.name]);
      return { id: result.insertId, name: input.name };
    },

    updateCategory: async (_: any, { input }: { input: any }) => {
      await db.query("UPDATE categories SET name=? WHERE id=?", [input.name, input.id]);
      const [rows] = await db.query<RowDataPacket[]>("SELECT * FROM categories WHERE id=?", [input.id]);
      return rows[0];
    },
    ...
    addProduct: async (_: any, { input }: { input: any }) => {
      const [result] = await db.query<ResultSetHeader>(
        `INSERT INTO products (name,price,restaurant_id,category_id,description,image_url) VALUES (?,?,?,?,?,?)`,
        [input.name, input.price, input.restaurantId, input.categoryId, input.description, input.imageUrl]
      );
      return { id: result.insertId, ...input };
    },
    ...
    updateProduct: async (_: any, { input }: { input: any }) => {
      await db.query(
        `UPDATE products
         SET name=?, price=?, restaurant_id=?, category_id=?, description=?, image_url=?
         WHERE id=?`,
        [input.name, input.price, input.restaurantId, input.categoryId, input.description, input.imageUrl, input.id]
      );
